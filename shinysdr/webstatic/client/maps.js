...
define(['./values', './gltools', './widget', './widgets', './events'], function (values, gltools, widget, widgets, events) {
  ...
  function GeoMap(config) {
    ...
    addLayer('Station Position', {
      featuresCell: config.index.implementing('shinysdr.devices.IPositionedDevice'),
    ...
    plugins.forEach(function(pluginFunc) {
      // TODO provide an actually designed and sensible interface
      pluginFunc(db, scheduler, config.index, addLayer, addModeLayer);
    });
  }
