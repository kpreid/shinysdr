...
define([
  ...
  class Menu {
    constructor(widgetContext, widgetCtor, target) {
      ...
      const menuContext = widgetContext.forMenu(function closeCallback() {
        dialog.close();
      });
    
      const widgetHandle = createWidgetExt(menuContext, widgetCtor, innerElement, new ConstantCell(target, anyT));
    ...
    openAt(targetElOrEvent) {
      const dialog = menuDialog.get(this);
      ...
      if (targetElOrEvent && targetElOrEvent.nodeType) {
        const dialogCR = dialog.getBoundingClientRect();
        const targetCR = targetElOrEvent.getBoundingClientRect();
        dialog.style.left = (
