...
class TestDependencyTester(unittest.TestCase):
    ...
    def test_module_ok(self):
        self.t.check_module('shinysdr.i.test_dependencies', '<dep name>')
    ...
    def test_module_broken_import(self):
        self.t.check_module('shinysdr.i.test_dependencies_cases.imports', '<dep name>')
        if six.PY2:
            self.assertEqual(self.t.report(),
    ...
    def test_module_broken_other(self):
        ...
        self.assertEqual(self.t.report(),
            'The following libraries/programs are not installed correctly:\n\t<dep name>  (Check: shinysdr.i.test_dependencies_cases.misc failed to import (boo).)\nPlease (re)install current versions.')
    
    def test_attr_ok(self):
        self.t.check_module_attr('shinysdr.i.test_dependencies', '<dep name>', 'TestDependencyTester')
        self.assertEqual(self.t.report(), None)
    
    def test_attr_path_ok(self):
        self.t.check_module_attr('shinysdr.i.test_dependencies', '<dep name>', 'TestDependencyTester.test_attr_path_ok')
        self.assertEqual(self.t.report(), None)
    
    def test_attr_missing(self):
        self.t.check_module_attr('shinysdr.i.test_dependencies', '<dep name>', 'nonexistent_attr')
        self.assertEqual(self.t.report(),
            'The following libraries/programs are too old:\n\t<dep name>  (Check: shinysdr.i.test_dependencies.nonexistent_attr not present.)\nPlease (re)install current versions.')
    
    # note: 'broken attr' (hasattr true but it raises on get) is theoretically possible but hard to cause
    
    def test_file_ok(self):
        self.t.check_jsdep_file(__file__, 'test_dependencies_cases/__init__.py', '<dep name>')
        self.assertEqual(self.t.report(), None)
